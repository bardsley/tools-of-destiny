...
<nav>
    <h1>Perk-o-later</h1>
    <p>Using D2 APi version {$manifest.version} containing {Object.keys($manifest.items).length} items</p>
</nav>
    ...
    <div class="columns">
        <div class="selector">
            ...
            {#each simplePlugs as Plugs, i}
                <div class="perkCol">
                {#each Plugs as Plug}
                    <a on:click|preventDefault={togglePerk} data-perkId="{Plug.id}" data-colId="{i}" class="perk" style="background-image: url('https://www.bungie.net{Plug.icon}')"></a>
                {/each}
                </div>
            {/each}
            </div>
