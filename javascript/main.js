var apps = [
        {   "name": "Braytech",                 "url": "https://bray.tech",
            ...
            "primary-colour": "#555", "secondary-colour": "#ccc", "logo": "", "text-colour": "#fff"
    ...
    document.addEventListener('DOMContentLoaded',event => {
        ...
        apps.forEach(app => {
            if(app.name && app.url) {
                ...
                if(app["primary-colour"]) { title.style["background-color"] = app["primary-colour"] }
                if(app["secondary-colour"]) { clone.style["background-color"] = app["secondary-colour"] }

                if(app.description) { clone.querySelector(".desc").innerText = app.description } else { clone.querySelector(".desc").remove() }
                if(app.tags && app.tags.length >0) { 
                    clone.querySelector(".tags").innerHTML = app.tags.sort().map(tag=>{ return `<span data-tag="${tag}" style="background-color: ${app["primary-colour"]}" class="tag ${tag}">${tag}</span>` }).join(' ') 
